...
The frontend gate enforces exact Node/npm versions, authentic committed-lockfile verification followed by `npm ci`, delivery/toolchain verification, generated OpenAPI drift detection, strict `vue-tsc`, 52 Vitest cases, the Vite production build and browser-bundle audit. v27.39.5 browser acceptance retains 48 strict Playwright scenarios. A plain Maven run without generated Vue assets is not the complete v27.39.5 release path. CI, Docker and staging use the same frontend boundary.
