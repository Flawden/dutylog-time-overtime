...
Status: Vue app-shell ownership v1, DutyLog v27.35.1.

## Delivery, generated API and diagnostics foundation (v27.35.1)
...
└── features            bounded product domains added from v27.35.1 onward
```
...
Vue Router still uses memory history. The released hash route remains authoritative during v27.35.1. Vue navigation calls the named `DutyLogLegacyPlatform.navigate(view)` capability; legacy routing publishes the new frozen snapshot back through `subscribe(listener)`.
