# v27.42.3 browser bundle source-contract alignment

Exact v27.41.8 frontend validation passed, then Java 17 Maven ran 791 tests and failed only because `VueCalendarTimelineMigrationFrontendContractTest` still searched for retired `budget.maxBytes`. v27.42.3 aligns that compile-gated source contract with the six segmented entry/per-chunk/total raw+gzip budget keys; the actual audit limits and Vite chunk architecture are unchanged.
