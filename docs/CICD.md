...
## v27.36.8 static-contract alignment on the existing single-pass boundary
...
- Pushes to `test` run the full `Deploy staging / validate` path exactly once, then build, verify and deploy the immutable image; v27.36.8 adds no second workflow or bypass.
