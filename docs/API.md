...
`v27.45.1` adds owner-scoped `GET /api/v1/workdays/{date}` under the existing Shifts module boundary. The `WorkdayTruth` response joins the dated shift, scheduled unpaid break, base norm, Production Calendar required norm, existing Time Compensation projection and explicit Actual Work intervals without creating another user-entered truth. Actual Work input/output now carries `breakMinutes`; its net `workedMinutes` drives idempotent ordinary-overtime reconciliation into the existing FIFO authority. Existing Production Calendar and Actual Work mutation APIs remain the Calendar day commands. OpenAPI is **130 operations / 136 schemas** with hash `bb672251a454`; Flyway advances to V50.
