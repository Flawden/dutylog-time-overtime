...
`v27.45.1` adds owner-scoped `GET /api/v1/workdays/{date}` under the existing Shifts module boundary. The `WorkdayTruth` response joins the dated shift, base norm, Production Calendar required norm, existing Time Compensation projection and explicit Actual Work intervals without creating another persistence authority. Existing Production Calendar and Actual Work mutation APIs are reused by Calendar's native day surface. OpenAPI is **130 operations / 136 schemas** with hash `e78a6253744d`; Flyway remains V49.
