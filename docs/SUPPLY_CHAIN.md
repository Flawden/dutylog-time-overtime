...
v27.41.5 adds no dependency and preserves the exact reviewed Node/npm dependency graph; only the raw browser-bundle audit ceiling is rebaselined to 810000 B while gzip remains capped at 250000 B. CI must remain green before merging an update. Review release notes and run the manual smoke checklist for framework, database, browser-runner or container-major updates.
