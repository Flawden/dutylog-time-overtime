...
The committed frontend graph was generated by exact Node `20.18.1` and npm `10.8.2`, contains registry tarballs, SHA-512 integrity and dependency/peer edges, and is the only normal input to `npm ci`. Validation verifies that graph, checks local launchers and `npm ls --all`, then executes OpenAPI drift, `vue-tsc`, 43 Vitest cases, Vite and the browser-bundle audit before Maven. `v27.37.2` reuses the exact dependency graph and adds no package; the audit now also enforces reviewed raw/gzip JavaScript budgets from `frontend/browser-bundle-budget.json`.
