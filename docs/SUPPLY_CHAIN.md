...
v27.41.4 adds no dependency and preserves the exact reviewed Node/npm dependency graph. CI must remain green before merging an update. Review release notes and run the manual smoke checklist for framework, database, browser-runner or container-major updates.
