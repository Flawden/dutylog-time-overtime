...
The committed frontend graph was generated by exact Node `20.18.1` and npm `10.8.2`, contains registry tarballs, SHA-512 integrity and dependency/peer edges, and is the only normal input to `npm ci`. Validation verifies that graph, checks local launchers and `npm ls --all`, then executes OpenAPI drift, `vue-tsc`, 26 Vitest cases, Vite and the browser-bundle audit before Maven. v27.36.4 changes no dependency resolution: the exact committed graph is reused while diagnostic artifact storage becomes compact, short-lived and non-blocking.
