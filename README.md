...
`v25.2 — module-aware day panel`

Продолжение модульного этапа: панель выбранного дня теперь собирается с учётом включённых модулей. Блоки заметок, задач, переработок и важных дней скрываются через единый registry, календарные маркеры больше не показывают данные выключенных модулей, а пользователь видит мягкую подсказку, что часть блоков скрыта настройками.
...
Полировка англоязычного интерфейса: доработан перевод профиля, подсказок, активных устройств, Telegram-блока, системных сообщений и динамических статусов. Динамические строки теперь проходят через словарь приложения, а не остаются русским hardcode.

`v24.0.1 — shift settings UX and important-day refresh`

Полировочный патч после i18n: плюсик создания смены из панели дня теперь ведёт прямо в раскрытый раздел `Смены`, удаление важного дня сразу обновляет счётчик выбранного дня, а форма создания смены разделяет `Календарь, ч` как короткую метку и `Норма, ч` как значение для расчёта переработки.

`v24.0 — language switch / i18n`

В этой версии добавлена смена языка интерфейса: русский и английский. Настройка хранится в профиле пользователя (`language_preference`) и имеет локальный fallback для страницы входа и PWA-оболочки. Перевод сделан безопасным словарём приложения: пользовательский CSS/JS не используется, язык не влияет на роли, тарифы и бизнес-логику.
