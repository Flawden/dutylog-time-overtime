...
Для частых ситуаций добавлены быстрые сценарии: `+2ч после смены`, `+4ч после смены`, `остался в ночь`, `с начала смены + ночь`, `обычная смена целиком`. Они берут выбранный день и параметры смены, заполняют поля переработки как заготовку, а пользователь перед начислением может поправить время, обед, план или причину.
...
- Встроенные смены получили базовые настройки: дневная `06:30–17:00`, ночная `20:00–08:00`, выходной `0 ч`.
