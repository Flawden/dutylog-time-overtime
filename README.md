...
## Текущая версия: v27.17.1 — Calendar & Notes Quality Hotfix

**v27.17.1** закрывает реальные дефекты, найденные после первой desktop/mobile приёмки календаря:

- заметки внутри узкой панели выбранного дня теперь ориентируются на ширину собственного контейнера, а не всего окна браузера;
- список заметок, действия, вкладки и редактор складываются в безопасный вертикальный поток без горизонтального вылета;
- важные даты отображаются в отдельной зоне **«Весь день»**;
- форма важной даты автоматически получает день, выбранный в календаре;
- время доставки напоминания больше не превращает важную дату в ложное событие поверх смены;
- напоминания проецируются по фактической дате `displayAt` / `remindAt`, а не по дате исходной сущности;
- поле времени срока задачи принимает любую минуту, например `17:41`;
- Month / Week / Day, Today Dashboard и полный редактор дня сохраняют прежнюю бизнес-логику.
...
Следующий практический шаг — пропустить v27.17.1 через полный Maven и Playwright gate, подтвердить `/actuator/info` на staging и повторить desktop/mobile приёмку заметок и Day view. После этого начинается **v27.17.2 — UI Core & Workspace Foundation**.
