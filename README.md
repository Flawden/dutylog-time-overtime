...
- [`docs/PRODUCTION_LAUNCH.md`](docs/PRODUCTION_LAUNCH.md) — короткий сценарий первого запуска v22.2 на VPS.
...
В этой версии публичная регистрация управляется администратором из раздела `Система`. Backend проверяет настройку при каждом `POST /api/auth/register`, поэтому скрытие вкладки регистрации на странице входа — только удобство, а реальная защита находится на сервере. Стартовый администратор по-прежнему создаётся только через `DUTYLOG_ADMIN_USERNAME` и `DUTYLOG_ADMIN_PASSWORD`. Клиент по-прежнему web/PWA внутри Spring Boot-монолита; отдельного native mobile-приложения в v22.2 нет.


## Служебный профиль администратора

Диагностика не показывается в обычных пользовательских настройках. Администратор видит в шапке кнопку `Система`, где доступны версия интерфейса и сервера, состояние БД, Service Worker, Telegram-интеграция, переключатель публичной регистрации и безопасный отчёт без секретов.
...
Администраторы через UI не создаются. Стартовый администратор остаётся только bootstrap-механизмом через env, чтобы не открывать лишнюю опасную поверхность.
