...
## Текущая версия: v27.18.0 — Overtime Next

**v27.18.0** переводит переработки на единый DutyLog UI Core:

- крупный доступный остаток, общее начисление, использование и доля списания видны сразу;
- FIFO больше не спрятан в таблице: интерфейс показывает, какой остаток будет списан следующим;
- появились быстрые периоды «Месяц / Год / Всё время» и понятная подпись текущего диапазона;
- динамика начислений и списаний группируется по дням или месяцам без тяжёлой chart-библиотеки;
- на телефоне широкая таблица заменена подробными карточками начислений и списаний;
- на desktop сохранена профессиональная таблица со всей provenance-детализацией;
- быстрые формы начисления/списания, legacy-миграция и CSV/Excel-экспорт сохранены.

Backend domain API и схема данных не менялись; Flyway остаётся **V1–V36**. Автоматическая база: **96 Java-тестовых классов, 500 `@Test` методов и 26 Playwright browser scenarios**.
...
Следующий практический шаг — пропустить v27.18.0 через полный Maven и Playwright gate, подтвердить `/actuator/info` на staging и вручную проверить desktop-таблицу, мобильные карточки, FIFO-очередь и экспорт. После этого начинается **v27.19.0 — Tasks & Inbox Next**.
