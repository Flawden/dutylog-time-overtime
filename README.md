> Current release: **v27.11.3 — Shift Template & Reminder Timezone Hotfix**.
...
## Текущая версия: v27.11.3 — Shift Template & Reminder Timezone Hotfix

Предыдущий функциональный релиз: **v27.11.0 — Shift Occurrences & Calendar Projection**. Предыдущие исправляющие релизы: **v27.11.1 — CI & Contract Hotfix** и **v27.11.2 — E2E Stability Hotfix**.

**v27.11.3** завершает единую временную модель: при смене канонического IANA-пояса DutyLog теперь перепроецирует не только уже назначенные абсолютные смены, но и шаблоны будущих смен. Поэтому шаблон `08:30–17:00 Asia/Yekaterinburg` после перехода в `Europe/Moscow` отображается и сохраняется как `06:30–15:00`, сохраняя те же реальные моменты и длительность.

Существующие назначенные смены перед изменением шаблонов фиксируются как неизменные `startInstant`/`endInstant`, поэтому история не переписывается. Выходной и шаблоны без точного времени остаются нетронутыми.

Напоминания перед сменой теперь вычисляются от абсолютного `shiftStartInstant`, а не от текущих локальных часов шаблона. Браузер, Android-контракт и Telegram получают один и тот же `remindAtInstant`; если проекция переносит смену на следующий день, напоминание получает новую локальную дату и время.

Flyway остаётся на **V33**. Текущая автоматическая база: **85 Java-тестовых классов, 446 `@Test` методов и 19 Playwright browser scenarios**.
...
Следующий практический шаг — пропустить v27.11.3 через полный Maven и Playwright gate, затем на staging проверить `08:30 GMT+5 → 06:30 GMT+3`, перенос поздней смены на следующий день, legacy-мастер и read-first Task Details после обновления PWA.
