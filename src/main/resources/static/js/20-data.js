...
const api = {
  ...
  async ledgerIntegrity(from, to) { const qs = new URLSearchParams({ from, to }); return jfetch(`/api/ledger-integrity?${qs.toString()}`); },
  async closeAccountingPeriod(month) { return jfetch(`/api/ledger-integrity/periods/${encodeURIComponent(month)}/close`, { method:"POST" }); },
  async reopenAccountingPeriod(month) { return jfetch(`/api/ledger-integrity/periods/${encodeURIComponent(month)}/reopen`, { method:"POST" }); },
  async addLedgerAdjustment(body) { return jfetch("/api/ledger-integrity/adjustments", { method:"POST", body }); },
  async actualWork(from, to) { const qs = new URLSearchParams({ from, to }); return jfetch(`/api/actual-work?${qs.toString()}`); },
  ...
  async previewLegacyOvertimeUsages(b = {}) { return jfetch("/api/overtime/legacy-usages/preview", { method:"POST", body:b }); },
  async migrateLegacyOvertimeUsages(b = {}) { return jfetch("/api/overtime/legacy-usages/migrate", { method:"POST", body:b }); },
