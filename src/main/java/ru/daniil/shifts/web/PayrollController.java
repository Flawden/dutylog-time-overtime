...
/** Payroll Foundation API. Read models are private/no-store and calculations are revisioned. */
@RestController
...
public class PayrollController {
    ...
    public ResponseEntity<PayrollPeriodDto> period(@PathVariable("month") String month, Principal principal) {
        return ResponseEntity.ok().cacheControl(CacheControl.noStore())
                .body(payroll.period(user(principal), month));
    }

    @PatchMapping("/settings")
    public PayrollSettingsDto updateSettings(
            @Valid @RequestBody(required = false) PayrollSettingsUpdateRequest request,
            Principal principal) {
    ...
    public ResponseEntity<PayrollAdjustmentDto> addAdjustment(
            @Valid @RequestBody(required = false) PayrollAdjustmentRequest request,
            Principal principal) {
