...
public class StatutoryPublicHolidayAuthorityService {
    ...
    public StatutoryPublicHolidayAuthorityService(
        ...
        this.regional =
                Objects.requireNonNull(
                        regional,
                        "Statutory public-holiday authority requires regional dataset authority"
                );
    }
    ...
    public Resolution resolve(
        ...
        if (!regionalDecision.ready()) {
            return Resolution.unresolved(
                    date,
                    regionalDecision.blockingReason()
            );
        }

        if (regionalDecision.nonWorkingPublicHoliday()) {
            return Resolution.regionalHoliday(
                    date,
                    jurisdictionFact,
                    regionalDecision
            );
        ...
        if (regionalDecision.provenNotPublicHoliday()) {
            return Resolution.regionalNotHoliday(
                    date,
                    jurisdictionFact,
                    regionalDecision
            );
        ...
        throw new IllegalStateException(
                "Ready regional statutory-holiday decision has unsupported status"
        );
    }
    ...
    public record Provenance(
            ...
            String regionalSourcePackSchema,
            String regionalSourcePackSha256,
            String regionalCompletenessEvidence,
        ...
        public Provenance {
            ...
            Objects.requireNonNull(
                    authorityKind,
                    "Statutory holiday authority kind is required"
            );
            ...
            if (authorityKind == AuthorityKind.FEDERAL_ARTICLE_112) {
                requireText(
                        holidayCode,
                        "Federal statutory holiday identity is required"
                );

                if (regionalDatasetId != null
                        || regionalDatasetFingerprint != null
                        || regionalDatasetComplete != null
                        || regionalSourcePackSchema != null
                        || regionalSourcePackSha256 != null
                        || regionalCompletenessEvidence != null
                        || regionalDateFactId != null) {
                    throw new IllegalArgumentException(
                            "Federal statutory holiday must not claim regional dataset identity"
                    );
                }
            }
            else {
                requireText(
                        regionCode,
                        "Regional statutory holiday provenance requires region"
                );

                if (regionalDatasetId == null
                        || regionalDatasetId <= 0L
                        || regionalDatasetFingerprint == null
                        || !regionalDatasetFingerprint.matches("[0-9a-f]{64}")
                        || regionalDatasetComplete == null
                        || !RegionalStatutoryHolidayDatasetService.SOURCE_PACK_SCHEMA_V1.equals(
                                regionalSourcePackSchema
                        )
                        || regionalSourcePackSha256 == null
                        || !regionalSourcePackSha256.matches("[0-9a-f]{64}")
                        || regionalCompletenessEvidence == null
                        || regionalCompletenessEvidence.isBlank()) {
                    throw new IllegalArgumentException(
                            "Regional statutory holiday provenance requires immutable dataset and source-pack identity"
                    );
                }
            }
        }
    ...
    public record Resolution(
        ...
        public Resolution {
            ...
            if (status == Status.NON_WORKING_PUBLIC_HOLIDAY
                    && (provenance == null
                    || provenance.holidayCode() == null
                    || provenance.holidayCode().isBlank())) {
                throw new IllegalArgumentException(
                        "Positive statutory holiday requires holiday identity"
                );
            }

            if (status == Status.NOT_NON_WORKING_PUBLIC_HOLIDAY
                    && (provenance == null
                    || provenance.authorityKind() != AuthorityKind.REGIONAL_DATASET
                    || !Boolean.TRUE.equals(
                            provenance.regionalDatasetComplete()
                    )
                    || provenance.holidayCode() != null
                    || provenance.regionalDateFactId() != null)) {
                throw new IllegalArgumentException(
                        "Negative statutory holiday requires complete regional dataset provenance"
                );
            }
        }
        ...
        public static Resolution federalHoliday(
            ...
            return new Resolution(
                    ...
                    new Provenance(
                            ...
                            AuthorityKind.FEDERAL_ARTICLE_112,
                            federal.legalRegime(),
                            federal.legalBasis(),
                            federal.sourceRevision(),
                            federal.sourceReference(),
                            federal.holidayCode().name(),
                            null,
                            null,
                            null,
                            null,
        ...
        public static Resolution regionalHoliday(
            ...
            requireRegionalDecision(
                    regional
            );

            if (!regional.nonWorkingPublicHoliday()
                    || regional.holiday() == null) {
                throw new IllegalArgumentException(
                        "Regional holiday resolution requires positive regional fact"
                );
            }

            RegionalStatutoryHolidayDatasetService.DatasetProvenance dataset =
                    regional.provenance();

            RegionalStatutoryHolidayDatasetService.HolidayFact holiday =
                    regional.holiday();
            ...
            return new Resolution(
                    ...
                    new Provenance(
                            ...
                            jurisdiction.jurisdictionCode(),
                            jurisdiction.regionCode(),
                            AuthorityKind.REGIONAL_DATASET,
                            dataset.legalRegime(),
                            holiday.legalBasis(),
                            ...
                            dataset.fingerprint(),
                            dataset.complete(),
                            dataset.sourcePackSchema(),
                            dataset.sourcePackSha256(),
                            dataset.completenessEvidence(),
                            holiday.factId()
                    )
            );
        }
        ...
        public static Resolution regionalNotHoliday(
            ...
            if (!regional.provenNotPublicHoliday()
                    || regional.holiday() != null
                    || !regional.provenance().complete()) {
                throw new IllegalArgumentException(
                        "Regional negative resolution requires complete negative authority"
                );
            }
        ...
        private static void requireRegionalDecision(
                RegionalStatutoryHolidayDatasetService.Decision regional
        ) {
            Objects.requireNonNull(
                    regional,
                    "Regional statutory holiday resolution requires regional decision"
            );

            if (!regional.ready()
                    || regional.provenance() == null) {
                throw new IllegalArgumentException(
                        "Regional statutory holiday resolution requires ready dataset provenance"
                );
            }
        }
