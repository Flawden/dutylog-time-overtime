...
test('partial time off keeps the planned shift and spends the unified overtime bank', async ({ page }) => {
  ...
  await page.evaluate(async date => {
    ...
    const response = await fetch('/api/overtime/credits', {
      ...
      body:JSON.stringify({ date, hours:8, reason:'E2E opening overtime bank' })
    });
  ...
  expect(created.linkedOvertimeUsageId).toBeTruthy();
