...
test('Vue foundation boots the released app shell inside the same-origin Spring session', async ({ page }) => {
  ...
  await expect(page.locator('#dutylog-vue-root')).toHaveAttribute('data-vue-version', '27.40.8');
  ...
  expect(diagnostics).toMatchObject({ releaseVersion: '27.40.8', route: 'calendar', fatal: null });
