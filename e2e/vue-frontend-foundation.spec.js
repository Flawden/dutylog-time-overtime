...
test('Vue foundation boots the released app shell inside the same-origin Spring session', async ({ page }) => {
  ...
  await expect(page.locator('#dutylog-vue-root')).toHaveAttribute('data-vue-version', '27.36.5');
  ...
  expect(platform).toMatchObject({
    ...
    legacyVersion: '27.36.5',
    snapshot: {
      releaseVersion: '27.36.5',
      architecture: 'vue-shell-v1',
      phase: 'ready',
  ...
  expect(diagnostics).toMatchObject({ releaseVersion: '27.36.5', route: 'calendar', fatal: null });
