...
test('Vue foundation boots the released app shell inside the same-origin Spring session', async ({ page }) => {
  ...
  await expect(page.locator('#dutylog-vue-root')).toHaveAttribute('data-vue-version', '27.38.13');
  ...
  expect(platform).toMatchObject({
    ...
    legacyVersion: '27.38.13',
    snapshot: {
      releaseVersion: '27.38.13',
      architecture: 'vue-shell-v1',
      phase: 'ready',
  ...
  expect(diagnostics).toMatchObject({ releaseVersion: '27.38.13', route: 'calendar', fatal: null });
