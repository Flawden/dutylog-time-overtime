...
test('PWA activation removes a previous release cache before claiming the v27.38.8 shell', async ({ browser, baseURL }) => {
  const context = await browser.newContext({ baseURL, locale: 'ru-RU', serviceWorkers: 'allow' });
  const page = await context.newPage();
  try {
    // Seed the synthetic previous cache before visiting login.html; login registers
    // the current service worker and would otherwise race the precondition itself.
    ...
    expect(cacheNames.some(name => name.startsWith('dutylog-shell-v27.38.8-'))).toBe(true);
