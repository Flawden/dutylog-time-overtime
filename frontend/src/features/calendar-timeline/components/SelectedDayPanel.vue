  ...
  <aside id="panel" class="card vue-selected-day-panel" data-vue-selected-day-panel>
    ...
    <details id="accSched" class="acc dayPanelModule" data-day-module="shifts" :open="isOpen('accSched')" @toggle="toggleSection('accSched', $event)">
      ...
      <div class="accB"><div id="tplBox" class="tplBox">
        ...
        <div id="tplPreview" class="schedulePreview" :hidden="!schedulePreview"><b>{{ schedulePreviewText }}</b><div v-for="item in schedulePreview?.items?.slice(0, 14) ?? []" :key="item.date" class="vue-schedule-preview-row"><span>{{ item.date }}</span><span>{{ item.shiftTypeName || 'выходной' }}</span><small>{{ item.action }}</small></div></div>
