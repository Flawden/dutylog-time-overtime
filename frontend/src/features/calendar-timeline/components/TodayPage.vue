    ...
    <div class="todayDashboard">
      ...
      <div class="todayDashboardGrid">
        <template v-for="widget in todayWidgets" :key="widget">
          ...
          <section v-else id="todayUpcomingCard" class="card todayUpcomingCard"><div class="todayCardHead"><span class="todayCardIcon">★</span><div class="todayCardHeading"><small>Не пропустить</small><h3 id="todayUpcomingTitle">Ближайшие даты</h3></div><button id="todayOpenImportant" class="todayCardLink" type="button" @click="navigateHashRoute('important')">Все даты</button></div><div id="todayUpcomingList" class="todayUpcomingList"><button v-for="item in upcoming" :key="item.id" type="button" class="todayUpcomingRow" @click="openImportantDetails(item.id)"><span class="todayUpcomingDot" :style="{ '--event-color': item.color || 'var(--accent)' }"></span><span><b>{{ item.icon || '★' }} {{ item.title }}</b><small>{{ dateLabel(item.date, language, { day:'numeric', month:'short' }) }}</small></span></button><p v-if="!upcoming.length">Ближайших дат нет.</p></div></section>
        </template>
      </div>
    </div>
  </section>
