...
function openTask(): void { props.bridge.openTaskCreate(workDate.value); }
  ...
  <section id="view-today" class="view todayView vue-today-view" data-vue-domain-route="today" data-vue-domain-owner="calendar-timeline">
    <div class="todayDashboard">
      ...
      <div class="todayDashboardGrid">
        ...
        <section id="todayTasksCard" class="card todayTasksCard"><div class="todayCardHead"><span class="todayCardIcon">✓</span><div class="todayCardHeading"><small>План дня</small><h3 id="todayTasksTitle">Задачи</h3></div><span id="todayTaskCount" class="todayCountPill">{{ openTasks.length }}</span></div><div id="todayTaskList" class="todayTaskList"><button v-for="task in openTasks.slice(0,5)" :key="task.id" type="button" class="todayTaskMain" @click="props.bridge.openTaskDetails(task.id)"><b>{{ task.text }}</b><small>{{ task.dueTime || task.priority }}</small></button><p v-if="!openTasks.length">На сегодня открытых задач нет.</p></div><button id="todayOpenTasks" class="todayWideLink" type="button" @click="props.bridge.navigate('tasks')">Показать все задачи →</button></section>
        <section id="todayUpcomingCard" class="card todayUpcomingCard"><div class="todayCardHead"><span class="todayCardIcon">★</span><div class="todayCardHeading"><small>Не пропустить</small><h3 id="todayUpcomingTitle">Ближайшие даты</h3></div><button id="todayOpenImportant" class="todayCardLink" type="button" @click="props.bridge.navigate('important')">Все даты</button></div><div id="todayUpcomingList" class="todayUpcomingList"><button v-for="item in upcoming" :key="item.id" type="button" class="todayUpcomingRow" @click="props.bridge.openImportantDetails(item.id)"><span class="todayUpcomingDot" :style="{ '--event-color': item.color || 'var(--accent)' }"></span><span><b>{{ item.icon || '★' }} {{ item.title }}</b><small>{{ dateLabel(item.date, language, { day:'numeric', month:'short' }) }}</small></span></button><p v-if="!upcoming.length">Ближайших дат нет.</p></div></section>
      </div>
    </div>
  </section>
</template>
