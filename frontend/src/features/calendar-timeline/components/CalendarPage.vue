...
async function openTimelineEvent(event: { type: string; actionId: number | null }): Promise<void> {
  if (event.type === "task" && event.actionId != null) props.bridge.openTaskDetails(event.actionId);
  else if (event.type === "important" && event.actionId != null) props.bridge.openImportantDetails(event.actionId);
      ...
      <div id="layout" class="layout">
        <div class="card vue-calendar-month-card">
          ...
          <div id="grid" class="grid" :aria-busy="loading ? 'true' : 'false'">
            <button v-for="date in gridDates" :key="date" type="button" class="cell" :class="{ sel: date === focusDate, todayCell: date === workDate, outside: !inFocusMonth(date), hasVacation: cellFacts(date).absences.length }" :data-date="date" @click="chooseDate(date)"><span class="num">{{ dayNumber(date) }}</span><span v-if="cellFacts(date).day?.dayEmoji" class="dayEmoji">{{ cellFacts(date).day?.dayEmoji }}</span><span v-if="cellFacts(date).shift" class="shift" :style="{ color: cellFacts(date).shift?.color }">{{ cellFacts(date).shift?.name }}</span><span v-if="cellFacts(date).day?.notes?.length || cellFacts(date).day?.note" class="ear"></span><span v-for="absence in cellFacts(date).absences.slice(0,1)" :key="absence.periodId" class="vacationMark" :style="{ background: absence.typeColor }">●</span><span v-for="item in cellFacts(date).important.slice(0,1)" :key="item.id" class="importantMark">★</span><span v-if="cellFacts(date).tasks.some(task => !task.done)" class="taskMark">!</span><span v-for="item in cellFacts(date).layers.slice(0,2)" :key="`${item.layer.id}-${item.entry.sourceDate}`" class="calendarLayerChip" :style="{ '--layer-color': item.layer.color }">{{ item.layer.name }}</span></button>
          </div>
