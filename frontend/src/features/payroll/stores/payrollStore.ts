...
export const usePayrollStore = defineStore("dutylog-payroll", {
  ...
  actions: {
    async load(month: string): Promise<void> {
      ...
      this.error = "";
      try {
        const period = await api.period(month);
        ...
        if (sequence === loadSequence) this.loading = false;
      }
    },
    async saveSettings(body: PayrollSettingsInput): Promise<void> {
      await api.updateSettings(body);
      await this.load(this.month);
    },
    async addAdjustment(body: PayrollAdjustmentInput): Promise<void> {
      await api.addAdjustment(body);
      await this.load(this.month);
    },
