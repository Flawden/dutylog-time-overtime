...
VERSION="${DUTYLOG_RELEASE_VERSION:-27.35.1}"
ERRORS=0
STATIC_JS=(
  "js/10-core.js"
...
contains docs/API.md "# DutyLog API v27.35.1"
contains docs/RELEASE_CHECKLIST.md "Status: v27.35.1."
...
contains docs/ROADMAP.md "Current release: **v27.35.1 — Frontend Lockfile Executable Resolution Hotfix**"
...
contains docs/ROADMAP.md "Current release: **v27.35.1 — Frontend Lockfile Executable Resolution Hotfix**"
contains docs/REGRESSION_TEST_BASELINE.md "Current extension: v27.35.1"
...
contains docs/SECURITY_REVIEW.md "Status: v27.35.1."
...
contains docs/RELEASE_CHECKLIST.md "git tag -a v27.35.1"
...
contains docs/RELEASE_CHECKLIST.md "Status: v27.35.1."
contains docs/SECURITY_REVIEW.md "Status: v27.35.1."
contains docs/MODULE_CONTRACTS.md "Status: v27.35.1."
...
contains src/main/resources/static/index.html '/vue/dutylog-vue-app-shell.css?v=27.35.1'
contains src/main/resources/static/index.html '/vue/dutylog-vue-app-shell.js?v=27.35.1'
...
contains e2e/external-calendar-sync.spec.js 'PRODID:-//DutyLog//Time and Overtime 27.35.1//RU'
...
contains docs/MODULE_CONTRACTS.md "Vue delivery, generated API and executable resolution (v27.35.1)"
contains docs/SECURITY_REVIEW.md "v27.35.1 lockfile executable-resolution review"
...
contains frontend/package-lock.json '"lockfileVersion": 3'
...
contains docs/ENGINEERING_QUALITY_REGISTER.md 'Gate A открывается для `v27.36.0` только после этого зелёного прогона'
...
not_contains Dockerfile 'npm install'

  # v27.35.1 Frontend Lockfile Executable Resolution Hotfix
contains CHANGES.md "v27.35.1 — Frontend Lockfile Executable Resolution Hotfix"
contains README.md "v27.35.1 — Frontend Lockfile Executable Resolution Hotfix"
contains docs/FRONTEND_LOCKFILE_EXECUTABLE_RESOLUTION_HOTFIX_V27.35.1.md 'node_modules/.bin/vue-tsc'
contains docs/ROADMAP.md "v27.35.1 — Frontend Lockfile Executable Resolution Hotfix — current"
contains docs/REGRESSION_TEST_BASELINE.md "v27.35.1 Frontend Lockfile Executable Resolution extension"
contains docs/RELEASE_CHECKLIST.md "v27.35.1 Frontend Lockfile Executable Resolution Hotfix acceptance"
contains frontend/package-lock.json '"vue-tsc": "bin/vue-tsc.js"'
contains frontend/package-lock.json '"vitest": "vitest.mjs"'
contains frontend/package-lock.json '"vite": "bin/vite.js"'
contains frontend/scripts/verify-delivery-foundation.mjs 'executableContracts'
...
contains Dockerfile 'test -e node_modules/.bin/vue-tsc'
contains Dockerfile 'npm ls --all'
contains src/test/java/ru/daniil/shifts/web/FrontendLockfileExecutableResolutionHotfixTest.java 'class FrontendLockfileExecutableResolutionHotfixTest'
...
contains e2e/overtime-scenario-manager.spec.js "waitForApi(page, 'POST', '/api/overtime/preview', 200)"
...
contains src/test/java/ru/daniil/shifts/web/CalendarSyncControllerTest.java "PRODID:-//DutyLog//Time and Overtime 27.35.1//RU"
...
not_contains src/main/java/ru/daniil/shifts/module/DutyLogModules.java 'ModuleService.PAYROLL'
  ...
  ok "Playwright test baseline: 44"
else
  fail "expected 44 Playwright tests, found $E2E_TESTS"
fi

VITEST_TESTS=$(grep -R --include='*.spec.ts' -h -E '^[[:space:]]*it\(' frontend/src | wc -l | tr -d ' ')
if [[ "$VITEST_TESTS" == "16" ]]; then
  ok "Vitest case baseline: 16"
else
  fail "expected 16 Vitest cases, found $VITEST_TESTS"
fi
...
if [[ "$TEST_METHODS" == "665" ]]; then
  ok "test method baseline: 665"
else
  fail "expected 665 @Test methods, found $TEST_METHODS"
fi
if [[ "$TEST_CLASSES" == "138" ]]; then
  ok "test class baseline: 138"
else
  fail "expected 138 test classes, found $TEST_CLASSES"
fi
