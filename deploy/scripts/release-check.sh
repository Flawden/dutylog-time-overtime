...
VERSION="${DUTYLOG_RELEASE_VERSION:-27.11.4}"
ERRORS=0
STATIC_JS=(
  "js/10-core.js"
  "js/20-data.js"
...
contains docs/REGRESSION_TEST_BASELINE.md "Current extension: v27.11.4"
...
ok "Playwright test baseline: 20"
else
  fail "expected 20 Playwright tests, found $E2E_TESTS"
fi
...
if [[ "$TEST_METHODS" == "456" ]]; then
  ok "test method baseline: 456"
else
  fail "expected 456 @Test methods, found $TEST_METHODS"
fi
if [[ "$TEST_CLASSES" == "86" ]]; then
  ok "test class baseline: 86"
else
  fail "expected 86 test classes, found $TEST_CLASSES"
fi
