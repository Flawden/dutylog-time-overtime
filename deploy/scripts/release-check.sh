...
if [[ "$TEST_METHODS" == "1144" ]]; then
  ok "test method baseline: 1144"
else
  fail "expected 1144 @Test methods, found $TEST_METHODS"
fi
if [[ "$TEST_CLASSES" == "242" ]]; then
  ok "test class baseline: 242"
else
  fail "expected 242 test classes, found $TEST_CLASSES"
fi
